...
def precipitation():
    ...
    precipitate_results = session.query(Measurement.date, Measurement.prcp).\
                   filter(Measurement.date.between('2016-08-01', '2017-08-01')).all()

    precipitation= []
    for result in percipitate_results:
        row = {"date":"prcp"}
        row["date"] = result[0]
...
def cal_temps(start='start_date'):
...
def cal_temps_two(start='start_date', end='end_date'):      
    start_date = datetime.strptime('2016-08-01', '%Y-%m-%d').date()
